...
void SetUpFCPCameraApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FCPCameraApi> *api, NSString *messageChannelSuffix) {
  ...
  /// Initializes the camera with the given ID.
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        ...
        FCPPlatformImageFormatGroup arg_imageFormat = [GetNullableObjectAtIndex(args, 1) integerValue];
  ...
  /// Locks the camera capture to the current device orientation.
  {
    ...
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FCPPlatformDeviceOrientation arg_orientation = [GetNullableObjectAtIndex(args, 0) integerValue];
  ...
  /// Sets the video stabilization mode.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.camera_avfoundation.CameraApi.setVideoStabilizationMode", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FCPCameraApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setVideoStabilizationMode:completion:)], @"FCPCameraApi api (%@) doesn't respond to @selector(setVideoStabilizationMode:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FCPPlatformVideoStabilizationMode arg_mode = [GetNullableObjectAtIndex(args, 0) integerValue];
        [api setVideoStabilizationMode:arg_mode completion:^(FlutterError *_Nullable error) {
          callback(wrapResult(nil, error));
        }];
      }];
    } else {
  ...
  /// Sets the video stabilization mode.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.camera_avfoundation.CameraApi.isVideoStabilizationModeSupported", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FCPCameraApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(isVideoStabilizationModeSupported:completion:)], @"FCPCameraApi api (%@) doesn't respond to @selector(isVideoStabilizationModeSupported:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FCPPlatformVideoStabilizationMode arg_mode = [GetNullableObjectAtIndex(args, 0) integerValue];
        [api isVideoStabilizationModeSupported:arg_mode completion:^(NSNumber *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
...
- (void)deviceOrientationChangedOrientation:(FCPPlatformDeviceOrientation)arg_orientation completion:(void (^)(FlutterError *_Nullable))completion {
  ...
  [channel sendMessage:@[[NSNumber numberWithInteger:arg_orientation]] reply:^(NSArray<id> *reply) {
    if (reply != nil) {
      if (reply.count > 1) {
...
NSObject<FlutterMessageCodec> *FCPCameraEventApiGetCodec(void) {
  ...
  static dispatch_once_t sPred = 0;
  dispatch_once(&sPred, ^{
    FCPCameraEventApiCodecReaderWriter *readerWriter = [[FCPCameraEventApiCodecReaderWriter alloc] init];
    sSharedObject = [FlutterStandardMessageCodec codecWithReaderWriter:readerWriter];
  });
  return sSharedObject;
}
